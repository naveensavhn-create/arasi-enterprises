...
CREATE OR REPLACE FUNCTION public.promoter_list_my_customers()
 RETURNS TABLE(id uuid, email text, full_name text, phone text, address_line1 text, address_line2 text, city text, state text, postal_code text, country text, aadhaar_address text, has_aadhaar_docs boolean, kyc_status kyc_status, kyc_submitted_at timestamp with time zone, kyc_reviewed_at timestamp with time zone, kyc_review_notes text, created_at timestamp with time zone, membership_number text, membership_status text, member_display_id text, coupon_no text, plan_id uuid, plan_name text)
 LANGUAGE plpgsql
